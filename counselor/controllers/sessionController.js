...
exports.addSession = async (req, res) => {
  try {
    ...
    // Convert session date and time to IST
    const istSessionDate = convertToIST(parsedSessionDate);

    // Ensure session_date is not in the past (IST)
    if (istSessionDate < new Date().setHours(0, 0, 0, 0)) {
      return res
        .status(400)
        .send({ error: "Session date cannot be in the past" });
    }
    ...
    const startDateTime = getSessionDateTime(istSessionDate, parsedSessionTime);
    const endDateTime = getSessionDateTime(
      istSessionDate,
      parsedSessionTime + parsedSessionDuration
    );
  ...
  function convertToIST(date) {
    // Convert the given date to IST by adding 5 hours 30 minutes
