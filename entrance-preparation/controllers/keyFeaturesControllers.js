  ...
  try {
    ...
    if (!allKeyFeatures || allKeyFeatures.length === 0) {
      return res.status(404).json({
        message: "Key features not found for the specified institute",
      });
    }

    let key_feature_ids = allKeyFeatures.map(
      (keyFeature) => keyFeature.key_feature
    );

    // console.log(key_feature_ids)

    const { data } = await axios.get(
      `${BACKEND_URL}/admin/key-features-institute/key-features`,
      {
        params: {
...
exports.deleteKeyFeatures = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Key feature deleted successfully", deletedKeyFeature });
  } catch (error) {
...
exports.editKeyFeatures = async (req, res) => {
  try {
    ...
    const existing_key_features = await KeyFeatures.find({
      institute: institute_id,
    });
    ...
    if (!Array.isArray(new_key_features)) {
      return res
        .status(400)
        .json({ message: "Invalid or missing new_key_features array" });
    }

    // Delete key features not present in new_key_features
    for (const existingFeature of existing_key_features) {
      const found = new_key_features.find(
        (newFeature) =>
          newFeature._id.toString() === existingFeature.key_feature.toString()
      );
      if (!found) {
    ...
    for (const newFeature of new_key_features) {
      const found = existing_key_features.find(
        (existingFeature) =>
          existingFeature.key_feature.toString() === newFeature._id.toString()
      );
      if (!found) {
        const newKeyFeature = new KeyFeatures({
          institute: institute_id,
